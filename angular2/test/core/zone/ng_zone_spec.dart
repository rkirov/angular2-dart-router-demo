...
commonTests() {
  ...
  describe("run", () {
    it("should return the body return value from run", inject(
        [AsyncTestCompleter], (async) {
      macroTask(() {
        expect(_zone.run(() {
          return 6;
    ...
    it("should call onTurnStart and onTurnDone", inject([AsyncTestCompleter],
        (async) {
      macroTask(() {
    ...
    it("should not run onTurnStart and onTurnDone for nested Zone.run", inject(
        [AsyncTestCompleter], (async) {
      macroTask(() {
        _zone.run(() {
          _log.add("start run");
    ...
    it("should run a function outside of the angular zone", inject(
        [AsyncTestCompleter], (async) {
      macroTask(() {
    ...
    it("should call onTurnStart before executing a microtask scheduled in onTurnDone as well as " +
        "onTurnDone after executing the task", inject([AsyncTestCompleter],
            (async) {
      var ran = false;
      _zone.initCallbacks(onTurnStart: _log.fn("onTurnStart"), onTurnDone: () {
    ...
    it("should execute promises scheduled in onTurnStart before promises scheduled in run",
      ...
      _zone.initCallbacks(onTurnStart: () {
    ...
    it("should call onTurnStart and onTurnDone for promises created outside of run body",
      ...
      macroTask(() {
        _zone.runOutsideAngular(() {
          promise =
              PromiseWrapper.resolve(4).then((x) => PromiseWrapper.resolve(x));
        });
        _zone.run(() {
  ...
  describe("exceptions", () {
    it("should call the on error callback when it is defined", inject(
        [AsyncTestCompleter], (async) {
      macroTask(() {
        _zone.initCallbacks(onErrorHandler: logError);
    ...
    it("should call onError for errors from microtasks", inject(
        [AsyncTestCompleter], (async) {
      _zone.initCallbacks(onErrorHandler: logError);
    ...
    it("should call onError when onTurnDone throws and the zone is sync",
      ...
      _zone.initCallbacks(onErrorHandler: logError, onTurnDone: () {
        throw exception;
      });
      macroTask(() {
        _zone.run(() {});
