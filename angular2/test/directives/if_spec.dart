library angular2.test.directives.if_spec;
...
main() {
  describe("if directive", () {
    it("should work in a template attribute", inject([
      ...
      var html =
          "<div><copy-me template=\"if booleanCondition\">hello</copy-me></div>";
      tb.createView(TestComponent, html: html).then((view) {
        view.detectChanges();
        expect(DOM.querySelectorAll(view.rootNodes[0], "copy-me").length)
            .toEqual(1);
    ...
    it("should work in a template element", inject([
      ...
      var html =
          "<div><template [if]=\"booleanCondition\"><copy-me>hello2</copy-me></template></div>";
      tb.createView(TestComponent, html: html).then((view) {
        view.detectChanges();
        expect(DOM.querySelectorAll(view.rootNodes[0], "copy-me").length)
            .toEqual(1);
    ...
    it("should handle nested if correctly", inject([
      ...
      var html =
          "<div><template [if]=\"booleanCondition\"><copy-me *if=\"nestedBooleanCondition\">hello</copy-me></template></div>";
      tb.createView(TestComponent, html: html).then((view) {
        view.context.booleanCondition = false;
        view.detectChanges();
        expect(DOM.querySelectorAll(view.rootNodes[0], "copy-me").length)
    ...
    if (!IS_DARTIUM) {
      it("should not add the element twice if the condition goes from true to true (JS)",
        ...
        var html =
            "<div><copy-me template=\"if numberCondition\">hello</copy-me></div>";
        tb.createView(TestComponent, html: html).then((view) {
          view.detectChanges();
          expect(DOM.querySelectorAll(view.rootNodes[0], "copy-me").length)
              .toEqual(1);
      ...
      it("should not recreate the element if the condition goes from true to true (JS)",
        ...
        tb.createView(TestComponent, html: html).then((view) {
          ...
          view.context.numberCondition = 2;
          view.detectChanges();
          expect(DOM.hasClass(view.rootNodes[0].childNodes[1], "foo"))
              .toBe(true);
          async.done();
        });
      }));
