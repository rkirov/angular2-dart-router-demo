...
main() {
  describe("integration tests", () {
    it("should initialize DOM elements with the given form object", inject([
      ...
      var ctx =
          new MyComp(new ControlGroup({"login": new Control("loginValue")}));
    ...
    it("should update DOM element when rebinding the control name", inject([
      TestBed,
      AsyncTestCompleter
    ], (tb, async) {
      var ctx = new MyComp(new ControlGroup(
          {"one": new Control("one"), "two": new Control("two")}), "one");
      var t = '''<div [control-group]="form">
...
class MyComp {
  dynamic form;
  String name;
  MyComp([@Inject("form") form = null, @Inject("name") name = null]) {
    this.form = form;
    this.name = name;
  }
}
@Directive(
    ...
    hostListeners: const {"change": "handleOnChange(\$event.target.value)"},
    hostProperties: const {"value": "value"})
class WrappedValue {
  var value;
  Function onChange;
  WrappedValue(ControlDirective cd) {
