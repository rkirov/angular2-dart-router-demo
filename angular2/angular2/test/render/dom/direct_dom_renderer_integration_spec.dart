library angular2.test.render.dom.direct_dom_renderer_integration_spec;
...
import "package:angular2/src/render/api.dart"
    show
        ...
        EventDispatcher,
        DirectiveMetadata;
import "integration_testbed.dart"
    show IntegrationTestbed, LoggingEventDispatcher, FakeEvent;
...
main() {
  describe("DirectDomRenderer integration", () {
    ...
    createRenderer({urlData, viewCacheCapacity, shadowDomStrategy, templates}) {
    ...
    it("should create host views while using the given elements in place",
        inject([AsyncTestCompleter], (async) {
      createRenderer();
      renderer.createHostProtoView(someComponent).then((rootProtoView) {
        expect(rootProtoView.elementBinders[0].directives[0].directiveIndex)
            .toBe(0);
        var viewRefs =
    ...
    it("should create imperative proto views", inject([AsyncTestCompleter],
      ...
      createRenderer();
      renderer
          .createImperativeComponentProtoView("someRenderId")
          .then((rootProtoView) {
        expect(rootProtoView.elementBinders).toEqual([]);
        expect(rootProtoView.render.delegate.imperativeRendererId)
            .toBe("someRenderId");
        async.done();
      });
    }));
    it("should add a static component", inject([AsyncTestCompleter], (async) {
      ...
      renderer.createHostProtoView(someComponent).then((rootProtoView) {
        ...
        renderer.compile(template).then((pv) {
          renderer.mergeChildComponentProtoViews(
              rootProtoView.render, [pv.render]);
          renderer.createInPlaceHostView(null, rootEl, rootProtoView.render);
    ...
    it("should add a a dynamic component", inject([AsyncTestCompleter],
        (async) {
      createRenderer();
      renderer.createHostProtoView(someComponent).then((rootProtoView) {
        var template = new ViewDefinition(
            componentId: "someComponent", template: "hello", directives: []);
        renderer.compile(template).then((pv) {
          var rootViewRef = renderer.createInPlaceHostView(
              null, rootEl, rootProtoView.render)[0];
    ...
    it("should update element properties", inject([AsyncTestCompleter],
        (async) {
      createRenderer(
          templates: [
        new ViewDefinition(
    ...
    it("should add and remove views to and from containers", inject(
      ...
      compileRoot(someComponent).then((rootProtoView) {
        var viewRef = renderer.createInPlaceHostView(
            null, rootEl, rootProtoView.render)[1];
        var vcProtoViewRef = rootProtoView.elementBinders[
            0].nestedProtoView.elementBinders[0].nestedProtoView.render;
