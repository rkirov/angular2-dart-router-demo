library angular2.test.render.dom.shadow_dom.shadow_css_spec;

import "package:angular2/test_lib.dart"
    show describe, beforeEach, it, expect, ddescribe, iit, SpyObject, el;
import "package:angular2/src/render/dom/shadow_dom/shadow_css.dart"
    show ShadowCss;
...
main() {
  describe("ShadowCss", () {
    ...
    it("should handle complicated selectors", () {
      ...
      expect(s("one+two {}", "a")).toEqual("one[a] + two[a] {}");
      expect(s("one~two {}", "a")).toEqual("one[a] ~ two[a] {}");
      expect(s(".one.two > three {}", "a"))
          .toEqual(".one.two[a] > three[a] {}");
    ...
    it("should support polyfill-unscoped-rule", () {
      var css = s(
          ...
          "a");
      expect(StringWrapper.contains(css, "#menu > .bar {;background: blue;}"))
          .toBeTruthy();
      css = s(
          "polyfill-unscoped-rule {content: \"#menu > .bar\";background: blue;}",
          "a");
    ...
    it("should support polyfill-rule", () {
      var css = s("polyfill-rule {content: ':host.foo .bar';background: blue;}",
          "a", "a-host");
      expect(css).toEqual("[a-host].foo .bar {background: blue;}");
      css = s("polyfill-rule {content: \":host.foo .bar\";background: blue;}",
          "a", "a-host");
      expect(css).toEqual("[a-host].foo .bar {background: blue;}");
    });
