...
import "package:angular2/src/render/dom/shadow_dom/style_inliner.dart"
    show StyleInliner;
import "integration_testbed.dart" show IntegrationTestbed;

main() {
  describe("ShadowDom integration tests", () {
    var urlResolver, styleUrlResolver, styleInliner;
    var strategies = {
      "scoped": () => new EmulatedScopedShadowDomStrategy(
    ...
    StringMapWrapper.forEach(strategies, (strategyFactory, name) {
      describe('''${ name} shadow dom strategy''', () {
        ...
        createRenderer({templates, viewCacheCapacity}) {
        ...
        it("should support simple components", inject([AsyncTestCompleter],
            (async) {
          createRenderer(
              templates: [
            new ViewDefinition(
        ...
        it("should not show the light dom event if there is not content tag",
        ...
        it("should support dynamic components", inject([AsyncTestCompleter],
            (async) {
          createRenderer(
              templates: [
            new ViewDefinition(
          ...
          compileRoot(mainDir).then((rootPv) {
            compile("simple").then((simplePv) {
              var views =
                  renderer.createInPlaceHostView(null, rootEl, rootPv.render);
        ...
        it("should support multiple content tags", inject([AsyncTestCompleter],
            (async) {
          createRenderer(
              templates: [
            new ViewDefinition(
        ...
        it("should redistribute direct child viewcontainers when the light dom changes",
          ...
          compileRoot(mainDir).then((pv) {
            ...
            var vcProtoViewRef = pv.elementBinders[
                0].nestedProtoView.elementBinders[1].nestedProtoView.render;
        ...
        it("should support nested components", inject([AsyncTestCompleter],
            (async) {
          createRenderer(
              templates: [
            new ViewDefinition(
        ...
        it("should redistribute when the shadow dom changes", inject(
          ...
          compileRoot(mainDir).then((pv) {
            ...
            var vcProtoViewRef = pv.elementBinders[
                0].nestedProtoView.elementBinders[
        ...
        it("should support tabs with view caching", inject([AsyncTestCompleter],
            (async) {
          createRenderer(
              templates: [
            new ViewDefinition(
          ...
          compileRoot(mainDir).then((pv) {
            ...
            var pvRef = mainPv.elementBinders[0].nestedProtoView.elementBinders[
                0].nestedProtoView.render;
            expect(rootEl).toHaveText("()");
