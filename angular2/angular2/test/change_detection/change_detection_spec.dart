...
import "package:angular2/src/change_detection/proto_change_detector.dart"
    show JitProtoChangeDetector, DynamicProtoChangeDetector;

main() {
  describe("change detection", () {
    StringMapWrapper.forEach({
      "dynamic": (bindingRecords, [variableBindings = null,
          directiveRecords = null, registry = null,
          strategy = null]) => new DynamicProtoChangeDetector(registry,
          isBlank(bindingRecords) ? [] : bindingRecords,
          isBlank(variableBindings) ? [] : variableBindings,
          isBlank(directiveRecords) ? [] : directiveRecords, strategy),
      "JIT": (bindingRecords, [variableBindings = null, directiveRecords = null,
          registry = null, strategy = null]) => new JitProtoChangeDetector(
          registry, isBlank(bindingRecords) ? [] : bindingRecords,
          isBlank(variableBindings) ? [] : variableBindings,
          isBlank(directiveRecords) ? [] : directiveRecords, strategy)
    }, (createProtoChangeDetector, name) {
      if (name == "JIT" && IS_DARTIUM) return;
      var parser = new Parser(new Lexer());
      ast(String exp, [String location = "location"]) {
        return parser.parseBinding(exp, location);
      }
      dirs(List directives) {
        return new FakeDirectives(directives, []);
      }
      convertLocalsToVariableBindings(locals) {
        var variableBindings = [];
        var loc = locals;
        while (isPresent(loc)) {
          MapWrapper.forEach(
              loc.current, (v, k) => ListWrapper.push(variableBindings, k));
          loc = loc.parent;
        }
        return variableBindings;
      }
      createChangeDetector(String propName, String exp,
          [context = null, locals = null, registry = null]) {
        ...
        var pcd =
            createProtoChangeDetector(records, variableBindings, [], registry);
        var cd = pcd.instantiate(dispatcher);
        cd.hydrate(context, locals, null);
        return {"changeDetector": cd, "dispatcher": dispatcher};
      }
      executeWatch(String memo, String exp, [context = null, locals = null]) {
      ...
      describe('''${ name} change detection''', () {
        ...
        it("should report all changes on the first run including uninitialized values",
            () {
          expect(executeWatch("value", "value", new Uninitialized()))
              .toEqual(["value=null"]);
        });
        ...
        it("should support method calls", () {
          ...
          expect(executeWatch("m", "sayHi(\"Jim\")", person))
              .toEqual(["m=Hi, Jim"]);
        });
        ...
        it("should support chained method calls", () {
          ...
          expect(executeWatch("m", "a.sayHi(\"Jim\")", td))
              .toEqual(["m=Hi, Jim"]);
        });
        ...
        describe("keyed access", () {
          it("should support accessing a list item", () {
            expect(executeWatch("array[0]", "[\"foo\", \"bar\"][0]"))
                .toEqual(["array[0]=foo"]);
          });
          it("should support accessing a map item", () {
            expect(executeWatch("map[foo]", "{\"foo\": \"bar\"}[\"foo\"]"))
                .toEqual(["map[foo]=bar"]);
          });
        });
        it("should support interpolation", () {
          ...
          var pcd = createProtoChangeDetector(
              [BindingRecord.createForElement(ast, 0, "memo")]);
          var cd = pcd.instantiate(dispatcher);
        ...
        describe("change notification", () {
          ...
          describe("pipes", () {
            it("should pass a change record to the dispatcher", () {
              var registry =
                  new FakePipeRegistry("pipe", () => new CountingPipe());
              var person = new Person("bob");
              var c = createChangeDetector(
                  "name", "name | pipe", person, null, registry);
              var cd = c["changeDetector"];
              var dispatcher = c["dispatcher"];
              cd.detectChanges();
          ...
          describe("updating directives", () {
            var dirRecord1 = new DirectiveRecord(
                new DirectiveIndex(0, 0), true, true, DEFAULT);
            var dirRecord2 = new DirectiveRecord(
                new DirectiveIndex(0, 1), true, true, DEFAULT);
            var dirRecordNoCallbacks = new DirectiveRecord(
                new DirectiveIndex(0, 0), false, false, DEFAULT);
            updateA(String exp, dirRecord) {
              return BindingRecord.createForDirective(
                  ast(exp), "a", (o, v) => o.a = v, dirRecord);
            }
            updateB(String exp, dirRecord) {
              return BindingRecord.createForDirective(
                  ast(exp), "b", (o, v) => o.b = v, dirRecord);
            }
            var directive1;
            var directive2;
            beforeEach(() {
            ...
            it("should happen directly, without invoking the dispatcher", () {
              var pcd = createProtoChangeDetector(
                  [updateA("42", dirRecord1)], [], [dirRecord1]);
              var cd = pcd.instantiate(dispatcher);
            ...
            describe("onAllChangesDone", () {
              it("should be called after processing all the children", () {
                var pcd =
                    createProtoChangeDetector([], [], [dirRecord1, dirRecord2]);
                var cd = pcd.instantiate(dispatcher);
                cd.hydrate(null, null, dirs([directive1, directive2]));
                cd.detectChanges();
              ...
              it("should be called in reverse order so the child is always notified before the parent",
                ...
                td1 = new TestDirective(
                    () => ListWrapper.push(onChangesDoneCalls, td1));
                var td2;
                td2 = new TestDirective(
                    () => ListWrapper.push(onChangesDoneCalls, td2));
        ...
        describe("reading directives", () {
          ...
          it("should read directive properties", () {
            ...
            directive.a = "aaa";
            var pcd = createProtoChangeDetector(
                [BindingRecord.createForHostProperty(index, ast("a"), "prop")],
                null, [dirRecord]);
            var cd = pcd.instantiate(dispatcher);
        ...
        describe("Locals", () {
          it("should read a value from locals", () {
            var locals = new Locals(
                null, MapWrapper.createFromPairs([["key", "value"]]));
      ...
      describe("mode", () {
        ...
        describe("marking ON_PUSH detectors as CHECK_ONCE after an update", () {
          ...
          beforeEach(() {
            ...
            checkedDetector.mode =
                CHECKED; // this directive is a component with ON_PUSH change detection
            dirRecordWithOnPush = new DirectiveRecord(new DirectiveIndex(0, 0),
                false, false, ON_PUSH); // a record updating a component
          ...
          it("should set the mode to CHECK_ONCE when a binding is updated", () {
            var proto = createProtoChangeDetector(
                [updateDirWithOnPushRecord], [], [dirRecordWithOnPush]);
            var cd = proto.instantiate(null);
            cd.hydrate(null, null, directives);
            expect(checkedDetector.mode).toEqual(
                CHECKED); // evaluate the record, update the targetDirective, and mark its detector as CHECK_ONCE
            cd.detectChanges();
      ...
      describe("hydration", () {
        ...
        it("should destroy all active pipes during dehyration", () {
          ...
          var c = createChangeDetector(
              "memo", "name | pipe", new Person("bob"), null, registry);
          var cd = c["changeDetector"];
          cd.detectChanges();
          cd.dehydrate();
      ...
      describe("pipes", () {
        it("should support pipes", () {
          ...
          var c =
              createChangeDetector("memo", "name | pipe", ctx, null, registry);
          var cd = c["changeDetector"];
          var dispatcher = c["dispatcher"];
          cd.detectChanges();
...
class FakePipeRegistry extends PipeRegistry {
  ...
  Function factory;
  dynamic cdRef;
  FakePipeRegistry(pipeType, factory) : super({}) {
    /* super call moved to initializer */;
    this.pipeType = pipeType;
    this.factory = factory;
    this.numberOfLookups = 0;
  }
  get(String type, obj, cdRef) {
...
class Person {
  ...
  num age;
  Address address;
  Person(String name, [Address address = null]) {
    this.name = name;
...
class FakeDirectives {
  List directives;
  List detectors;
  FakeDirectives(List directives, List detectors) {
...
class TestDispatcher extends ChangeDispatcher {
  ...
  notifyOnBinding(binding, value) {
    ListWrapper.push(this.log,
        '''${ binding . propertyName}=${ this . _asString ( value )}''');
