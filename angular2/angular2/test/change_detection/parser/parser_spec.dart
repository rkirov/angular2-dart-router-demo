...
import "package:angular2/src/change_detection/parser/ast.dart"
    show Pipe, LiteralPrimitive;

class TestData {
  var a;
  var b;
  var fnReturnValue;
  TestData(a, b, fnReturnValue) {
    this.a = a;
    this.b = b;
...
main() {
  td([a = 0, b = 0, fnReturnValue = "constant"]) {
    return new TestData(a, b, fnReturnValue);
  }
  createParser() {
    return new Parser(new Lexer(), reflector);
  }
  parseAction(text, [location = null]) {
    return createParser().parseAction(text, location);
  }
  parseBinding(text, [location = null]) {
    return createParser().parseBinding(text, location);
  }
  parseTemplateBindings(text, [location = null]) {
    return createParser().parseTemplateBindings(text, location);
  }
  parseInterpolation(text, [location = null]) {
    ...
    describe("parseAction", () {
      describe("basic expressions", () {
        ...
        it("should parse strict equality expressions", () {
          ...
          expectEval("2==='3'").toEqual(identical(2, "3"));
          expectEval("2==='2'").toEqual(identical(2, "2"));
          expectEval("2!==3").toEqual(!identical(2, 3));
          expectEval("2!=='3'").toEqual(!identical(2, "3"));
          expectEval("2!=='2'").toEqual(!identical(2, "2"));
      ...
      keyValues(templateBindings) {
        return ListWrapper.map(templateBindings, (binding) {
          if (binding.keyIsVar) {
            return "#" +
                binding.key +
                (isBlank(binding.name) ? "" : "=" + binding.name);
          } else {
            return binding.key +
