      ...
      describe("pipes", () {
        ...
        it("should support pipes in bindings and bind config", inject([
          ...
          tb.createView(MyComp, context: ctx).then((view) {
            ctx.ctxProp = "a";
            view.detectChanges();
            var comp = view.rawView.locals.get(
                "comp"); // it is doubled twice: once in the binding, second time in the bind config
      ...
      it("should support template directives via `<template>` elements.",
        ...
        tb.createView(MyComp, context: ctx).then((view) {
          ...
          var childNodesOfWrapper =
              view.rootNodes[0].childNodes; // 1 template + 2 copies.
...
@View(renderer: "simple-imp-cmp-renderer")
class SimpleImperativeViewComponent {
  var done;
  SimpleImperativeViewComponent(ElementRef self, DirectDomRenderer renderer) {
    renderer.setImperativeComponentRootNodes(
        self.parentView.render, self.boundElementIndex, [el("hello imp view")]);
  }
}
...
class DynamicViewport {
  ...
  DynamicViewport(ViewContainerRef vc, Injector inj, Compiler compiler) {
    ...
    this.done = compiler.compileInHost(ChildCompUsingService).then((hostPv) {
      vc.create(hostPv, 0, inj.createChildFromResolved(
