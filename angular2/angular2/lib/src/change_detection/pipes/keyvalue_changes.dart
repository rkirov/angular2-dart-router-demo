...
import "package:angular2/src/facade/lang.dart"
    show stringify, looseIdentical, isJsObject, CONST;
import "pipe.dart"
    show WrappedValue, Pipe, PipeFactory; /**
 * @exportedAs angular2/pipes
 */

class KeyValueChangesFactory extends PipeFactory {
  @CONST() const KeyValueChangesFactory() : super();
  bool supports(obj) {
...
class KeyValueChanges extends Pipe {
  ...
  _reset() {
    if (this.isDirty) {
      KVChangeRecord record; // Record the state of the mapping
  ...
  _truncate(KVChangeRecord lastRecord, KVChangeRecord record) {
    while (!identical(record, null)) {
      ...
      var nextRecord = record._next; // todo(vicb) assert
      //assert((() {
      //  record._next = null;
      //  return true;
      //}));
