library angular2.src.change_detection.pipes.iterable_changes;
...
class IterableChangesFactory extends PipeFactory {
  @CONST() const IterableChangesFactory() : super();
  bool supports(obj) {
...
class IterableChanges extends Pipe {
  ...
  IterableChanges() : super() {
    ...
    this._length =
        null; /// Keeps track of the used records at any point in time (during & across `_check()` calls)
    this._linkedRecords =
        null; /// Keeps track of the removed records at any point in time during `_check()` calls.
  ...
  CollectionChangeRecord _mismatch(
      CollectionChangeRecord record, item, int index) {
    ...
    if (identical(record, null)) {
      ...
      previousRecord =
          record._prev; // Remove the record from the collection since we know it does not match the item.
      this._remove(record);
    ...
    if (!identical(record, null)) {
      ...
      if (!identical(record, null)) {
        ...
        record = this._addAfter(
            new CollectionChangeRecord(item), previousRecord, index);
      }
    }
    return record;
  } /**
  ...
  CollectionChangeRecord _insertAfter(CollectionChangeRecord record,
    ...
    CollectionChangeRecord next = identical(prevRecord, null)
        ? this._itHead
        : prevRecord._next; // todo(vicb)
    //assert(next != record);
...
class _DuplicateItemRecordList {
   ...
   */
  add(CollectionChangeRecord record) {
...
class _DuplicateMap {
  ...
  CollectionChangeRecord remove(CollectionChangeRecord record) {
    var key = getMapKey(record.item); // todo(vicb)
    //assert(this.map.containsKey(key));
    _DuplicateItemRecordList recordList = MapWrapper.get(
        this.map, key); // Remove the list of duplicates when it gets empty
  ...
  bool get isEmpty {
    return identical(MapWrapper.size(this.map), 0);
  }
  clear() {
