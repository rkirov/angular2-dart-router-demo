...
class Compiler {
  ...
  Compiler(DirectiveMetadataReader reader, CompilerCache cache,
      ...
      UrlResolver urlResolver, renderApi.Renderer renderer,
  ...
  _compile(DirectiveBinding componentBinding) {
    ...
    if (isPresent(template.renderer)) {
      var directives = [];
      pvPromise = this._renderer
          .createImperativeComponentProtoView(template.renderer)
          .then((renderPv) {
        return this._compileNestedProtoViews(
    ...
    var protoViewDone = (_) {
      ...
      ListWrapper.forEach(protoView.elementBinders, (eb) {
        if (isPresent(eb.componentDirective)) {
          ...
          ListWrapper.push(childComponentRenderPvRefs,
              isPresent(componentPv) ? componentPv.render : null);
        }
      });
      this._renderer.mergeChildComponentProtoViews(
