...
import "package:angular2/src/render/dom/view/view_hydrator.dart" as rvh;
...
Injector _rootInjector; // Contains everything that is safe to share between applications.
...
List<Binding> _injectorBindings(appComponentType) {
  return [
    ...
    bind(appComponentRefToken).toAsyncFactory((dynamicComponentLoader, injector,
        appElement, appComponentAnnotatedType, testability, registry) {
      // We need to do this here to ensure that we create Testability and
      // it's ready on the window for users.
      registry.registerApplication(appElement, testability);
      return dynamicComponentLoader.loadIntoNewLocation(
          appComponentAnnotatedType.type, null, appElement, injector);
    }, [
    ...
    bind(rc.Compiler).toClass(
        rc.DefaultCompiler), // TODO(tbosch): We need an explicit factory here, as
    // we are getting errors in dart2js with mirrors...
    bind(rvf.ViewFactory).toFactory(
        (capacity, eventManager, shadowDomStrategy) =>
            new rvf.ViewFactory(capacity, eventManager, shadowDomStrategy), [
      rvf.VIEW_POOL_CAPACITY,
      EventManager,
      ShadowDomStrategy
    ]),
    bind(rvf.VIEW_POOL_CAPACITY).toValue(10000),
    rvh.RenderViewHydrator,
    ProtoViewFactory, // TODO(tbosch): We need an explicit factory here, as
    // we are getting errors in dart2js with mirrors...
...
VmTurnZone _createVmZone(Function givenReporter) {
...
Future<ApplicationRef> bootstrap(Type appComponentType,
  ...
  zone.run(() {
    ...
    PromiseWrapper.then(appInjector.asyncGet(appComponentRefToken),
        (componentRef) {
      var appChangeDetector = internalView(
          componentRef.hostView).changeDetector; // retrieve life cycle: may have already been created if injected in root component
...
Injector _createAppInjector(
    Type appComponentType, List<Binding> bindings, VmTurnZone zone) {
