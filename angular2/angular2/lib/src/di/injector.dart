...
class Injector {
   ...
   */
  static List<ResolvedBinding> resolve(List bindings) {
   ...
   */
  static Injector resolveAndCreate(List bindings, {defaultBindings: false}) {
   ...
   *
   */
  Injector resolveAndCreateChild(List bindings) {
  ...
  List _resolveDependencies(Key key, ResolvedBinding binding, bool forceAsync) {
    try {
      var getDependency = (d) =>
...
class _SyncInjectorStrategy {
  ...
  instantiate(Key key) {
    ...
    if (binding.providedAsPromise) throw new AsyncBindingError(
        key); //add a marker so we can detect cyclic dependencies
    this.injector._markAsConstructing(key);
    var deps = this.injector._resolveDependencies(key, binding, false);
    return this._createInstance(key, binding, deps);
  }
  _createInstance(Key key, ResolvedBinding binding, List deps) {
    try {
...
class _AsyncInjectorStrategy {
  ...
  instantiate(Key key) {
    var binding = this.injector._getBinding(key);
    if (isBlank(
        binding)) return _notFound; //add a marker so we can detect cyclic dependencies
    this.injector._markAsConstructing(key);
  ...
  Future _errorHandler(Key key, e) {
    if (e is AbstractBindingError) e.addKey(key);
    return PromiseWrapper.reject(e);
  }
  _findOrCreate(Key key, ResolvedBinding binding, List deps) {
    try {
      var instance = this.injector._getInstance(key);
...
List _resolveBindings(List bindings) {
  ...
  for (var i = 0; i < bindings.length; i++) {
    ...
    if (unresolved is ResolvedBinding) {
      ...
      throw new InvalidBindingError(
          "BindingBuilder with " + unresolved.token + " token");
    } else {
...
List _createListOfBindings(flattenedBindings) {
  var bindings = ListWrapper.createFixedSize(Key.numberOfKeys + 1);
  MapWrapper.forEach(flattenedBindings, (v, keyId) => bindings[keyId] = v);
  return bindings;
}
Map _flattenBindings(List bindings, Map res) {
