...
import "package:angular2/src/facade/lang.dart"
    show Type, isBlank, isPresent, CONST;
...
class Dependency {
  ...
  List properties;
  Dependency(
      Key key, bool asPromise, bool lazy, bool optional, List properties) {
    this.key = key;
...
class ResolvedBinding {
  ...
  ResolvedBinding(Key key, Function factory, List<Dependency> dependencies,
      bool providedAsPromise) {
    this.key = key;
...
class BindingBuilder {
   ...
   * ```
   */
  Binding toFactory(Function factoryFunction, [List dependencies = null]) {
   ...
   */
  Binding toAsyncFactory(Function factoryFunction, [List dependencies = null]) {
    return new Binding(this.token,
        toAsyncFactory: factoryFunction, deps: dependencies);
  }
}
_constructDependencies(Function factoryFunction, List dependencies) {
  return isBlank(dependencies)
      ? _dependenciesFor(factoryFunction)
      : ListWrapper.map(dependencies, (t) => Dependency.fromKey(Key.get(t)));
}
List _dependenciesFor(typeOrFunc) {
  var params = reflector.parameters(typeOrFunc);
  if (isBlank(params)) return [];
  if (ListWrapper.any(
      params, (p) => isBlank(p))) throw new NoAnnotationError(typeOrFunc);
