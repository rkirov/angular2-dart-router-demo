library benchpress.test.metric.perflog_metric_spec;
...
main() {
  ...
  createMetric(perfLogs,
      [microMetrics = null, perfLogFeatures = null, forceGc = null]) {
    commandLog = [];
    ...
    var bindings = [
      ...
      bind(WebDriverExtension).toValue(
          new MockDriverExtension(perfLogs, commandLog, perfLogFeatures))
    ];
  ...
  describe("perflog metric", () {
    ...
    it("should describe itself based on the perfLogFeatrues", () {
      expect(sortedKeys(
              createMetric([[]], null, new PerfLogFeatures()).describe()))
          .toEqual(["pureScriptTime", "scriptTime"]);
      expect(sortedKeys(createMetric(
              [[]], null, new PerfLogFeatures(render: true, gc: false))
          .describe())).toEqual(["pureScriptTime", "renderTime", "scriptTime"]);
      ...
      expect(sortedKeys(createMetric(
              [[]], null, new PerfLogFeatures(render: true, gc: true), true)
          .describe())).toEqual([
        "forcedGcAmount",
        "forcedGcTime",
        "gcAmount",
    ...
    it("should describe itself based on micro metrics", () {
      var description =
          createMetric([[]], {"myMicroMetric": "someDesc"}).describe();
    ...
    describe("beginMeasure", () {
      ...
      it("should force gc and mark the timeline", inject([AsyncTestCompleter],
          (async) {
      ...
      describe("with forced gc", () {
        ...
        it("should restart after the forced gc if needed", inject(
          ...
          metric
              ...
              .then((data) {
            expect(commandLog[5])
                .toEqual(["timeEnd", "benchpress1", "benchpress2"]);
            async.done();
          });
        }));
      });
    ...
    describe("aggregation", () {
      aggregate(events, [microMetrics = null]) {
      ...
      it("should report a single interval", inject([AsyncTestCompleter],
          (async) {
        aggregate([
      ...
      it("should sum up multiple intervals", inject([AsyncTestCompleter],
          (async) {
        aggregate([
          eventFactory.start("script", 0),
          eventFactory.end("script", 5),
      ...
      it("should ignore not started intervals", inject([AsyncTestCompleter],
          (async) {
      ...
      it("should ignore not ended intervals", inject([AsyncTestCompleter],
          (async) {
      ...
      it("should ignore events from different processed as the start mark",
        ...
        var otherProcessEventFactory =
            new TraceEventFactory("timeline", "pid1");
        var metric = createMetric([
          [
            eventFactory.markStart("benchpress0", 0),
      ...
      it("should support scriptTime metric", inject([AsyncTestCompleter],
          (async) {
        aggregate([
          eventFactory.start("script", 0),
          eventFactory.end("script", 5)
      ...
      it("should support renderTime metric", inject([AsyncTestCompleter],
          (async) {
        aggregate([
      ...
      it("should support gcTime/gcAmount metric", inject([AsyncTestCompleter],
          (async) {
        aggregate([
      ...
      it("should support majorGcTime metric", inject([AsyncTestCompleter],
          (async) {
        aggregate([
          eventFactory.start("gc", 0, {"usedHeapSize": 2500}),
      ...
      describe("microMetrics", () {
        ...
        it("should report micro metric averages", inject([AsyncTestCompleter],
            (async) {
          aggregate([
...
class MockDriverExtension extends WebDriverExtension {
  ...
  MockDriverExtension(perfLogs, commandLog, perfLogFeatures) : super() {
  ...
  Future timeEnd(name, restartName) {
