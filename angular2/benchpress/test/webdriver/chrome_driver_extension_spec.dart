library benchpress.test.webdriver.chrome_driver_extension_spec;
...
import "package:angular2/src/facade/lang.dart" show Json, isBlank;
...
main() {
  describe("chrome driver extension", () {
    ...
    createExtension(
        [perfRecords = null, messageMethod = "Tracing.dataCollected"]) {
    ...
    describe("readPerfLog", () {
      ...
      it("should normalize times to ms and forward ph and pid event properties",
        ...
        createExtension([
          ...
          expect(events)
              .toEqual([normEvents.complete("script", 1.1, 5.5, null)]);
          async.done();
        });
      }));
      it("should normalize \"tdur\" to \"dur\"", inject([AsyncTestCompleter],
          (async) {
        var event =
            chromeTimelineEvents.create("X", "FunctionCall", 1100, null);
        event["tdur"] = 5500;
      ...
      it("should report begin timestamps", inject([AsyncTestCompleter],
          (async) {
      ...
      it("should report gc", inject([AsyncTestCompleter], (async) {
        createExtension([
          chromeTimelineEvents.start(
              "GCEvent", 1000, {"usedHeapSizeBefore": 1000}),
      ...
      it("should ignore major gc from different processes", inject(
          [AsyncTestCompleter], (async) {
        createExtension([
          chromeTimelineEvents.start(
              "GCEvent", 1000, {"usedHeapSizeBefore": 1000}),
      ...
      it("should match chrome browsers", () {
        expect(createExtension().supports({"browserName": "chrome"}))
            .toBe(true);
        expect(createExtension().supports({"browserName": "Chrome"}))
            .toBe(true);
      });
...
class MockDriverAdapter extends WebDriverAdapter {
  List _log;
  List _events;
  String _messageMethod;
  MockDriverAdapter(log, events, messageMethod) : super() {
